...
// GET /api/sessions/protected - View sessions (role-based access)
const getSessions = withAuth(async (req, res, { user, supabase }) => {
  try {
    ...
    return res.status(200).json({
      ...
      user_role: user.role,
      user_id: user.id,
      total_sessions: sessionsWithRooms.length
    });
...
// POST /api/sessions/protected - Create session (Teachers/Admins only)
const createSession = withTeacherOrAdmin(async (req, res, { user, supabase }) => {
  try {
    const { 
      lesson_id, 
      subject_type, 
