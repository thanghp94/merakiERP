...
const RequestsTab: React.FC<RequestsTabProps> = ({ employees }) => {
  ...
  const getRequestSummary = (request: Request) => {
    ...
    switch (request_type) {
      case 'nghi_phep':
        return `${formatDate(request_data.from_date || '')} - ${formatDate(request_data.to_date || '')} (${request_data.total_days || 0} ngày)`;
      case 'doi_lich':
        return `${formatDate(request_data.original_date || '')} → ${formatDate(request_data.new_date || '')}`;
      case 'tam_ung':
        return formatCurrency(request_data.amount || 0);
      case 'mua_sam_sua_chua':
        return `${request_data.item_name || ''} - ${formatCurrency(request_data.estimated_cost || 0)}`;
      default:
        return '';
    }
  };
  ...
  const columns = [
    {
      key: 'title',
      label: 'Tiêu đề',
      render: (value: any, request: Request) => (
        <div>
    ...
    },
    {
      ...
      render: (value: any, request: Request) => (
        <div>
          <div className="font-medium">{request.created_by?.full_name}</div>
          <div className="text-sm text-gray-500">{request.created_by?.position}</div>
        </div>
      )
    },
    {
      key: 'status',
      label: 'Trạng thái',
      render: (value: any, request: Request) => getStatusBadge(request.status)
    },
    {
      key: 'created_at',
      label: 'Ngày tạo',
      render: (value: any, request: Request) => formatDate(request.created_at)
    }
  ];
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg max-w-2xl w-full">
            ...
            <div className="space-y-4">
              ...
              <div>
                <strong>Loại:</strong> {REQUEST_TYPE_LABELS[selectedRequest.request_type]}
              </div>
              <div>
                <strong>Trạng thái:</strong> {getStatusBadge(selectedRequest.status)}
              </div>
              <div>
                <strong>Người tạo:</strong> {selectedRequest.created_by?.full_name}
              </div>
              <div>
