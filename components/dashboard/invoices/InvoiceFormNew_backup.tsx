...
export default function InvoiceFormNew({ onSubmit, onCancel, initialData }: InvoiceFormNewProps) {
  ...
  useEffect(() => {
    loadFormData();
    if (initialData) {
      // Map invoice_items to items if it exists
      const mappedData = {
        ...initialData,
        ...
        is_income: initialData.is_income !== undefined ? initialData.is_income : true,
        create_payment: false, // Don't auto-check payment creation for edits
        payment_amount: 0,
        payment_date: new Date().toISOString().split('T')[0],
        reference_number: ''
      };
