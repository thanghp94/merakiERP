...
const RequestsView: React.FC<RequestsViewProps> = ({ employee }) => {
  ...
  const fetchRequests = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`/api/employee-requests?employee_id=${employee.id}`);
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-600">Quản lý các yêu cầu nghỉ phép và xin phép</p>
        </div>
        <button
      ...
      {requests.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mb-4">Bạn chưa tạo yêu cầu nào.</p>
          <button
            onClick={() => setShowRequestForm(true)}
      ...
      {/* Request Form Modal - Reusing the same form logic from PersonalTab */}
      {showRequestForm && (
